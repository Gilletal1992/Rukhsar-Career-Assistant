...
st.title("🌸 Rukhsar Career + Finnish Assistant");st.caption("Biblical Studies · church/ecumenical career · Finland-only employment · Finnish 0 → A2.2 · funding · research radar")
...
if section=="📊 Command Centre":
    ...
    st.header("🇫🇮 Adaptive Daily Finnish Trainer · A1 → A2.2")
    st.markdown("<div class='capacity-note'><b>Study at your own capacity.</b> Choose a suggested workload, then complete as many rounds as you want. There is no daily lock. Every new round gives a different task and today's skill score is the average of all completed rounds.</div>",unsafe_allow_html=True)
    ...
    reading_bank=[("Minna asuu Helsingissä. Hän menee aamulla bussilla kouluun. Iltapäivällä hän käy kaupassa.","Miten Minna menee kouluun?",["Junalla","Bussilla","Autolla","Kävellen"],"Bussilla"),("Anna opiskelee suomea maanantaina ja keskiviikkona. Tiistaina hän on kotona.","Milloin Anna opiskelee suomea?",["Maanantaina ja keskiviikkona","Tiistaina","Perjantaina","Sunnuntaina"],"Maanantaina ja keskiviikkona"),("Rukhsar menee aamulla kouluun. Tunti alkaa kello yhdeksän. Hän syö lounasta kello kaksitoista.","Mihin aikaan tunti alkaa?",["Kahdeksalta","Yhdeksältä","Kymmeneltä","Kahdeltatoista"],"Yhdeksältä"),("Lauantaina Sara menee kirkkoon ja sen jälkeen kahvilaan ystävän kanssa.","Minne Sara menee ensin?",["Kahvilaan","Kauppaan","Kirkkoon","Kirjastoon"],"Kirkkoon"),("Pekalla on pieni perhe. Hänellä on vaimo ja yksi lapsi. He asuvat Espoossa.","Missä perhe asuu?",["Helsingissä","Espoossa","Vantaalla","Turussa"],"Espoossa")]
    listen_bank=[("Tänään menen kauppaan ja ostan maitoa, leipää ja omenoita.","Mitä henkilö ostaa?",["Maitoa, leipää ja omenoita","Kahvia ja teetä","Vaatteita","Kirjoja"],"Maitoa, leipää ja omenoita"),("Huomenna aamulla menen lääkärille. Sen jälkeen menen kouluun bussilla.","Minne henkilö menee ensin?",["Kouluun","Lääkärille","Kauppaan","Kotiin"],"Lääkärille"),("Sunnuntaina tapaamme ystäviä kahvilassa kello kaksi.","Missä he tapaavat?",["Kirkossa","Kahvilassa","Asemalla","Puistossa"],"Kahvilassa"),("Bussi lähtee kello kahdeksan ja saapuu keskustaan puoli yhdeksältä.","Mihin bussi saapuu?",["Kouluun","Keskustaan","Lentokentälle","Kotiin"],"Keskustaan"),("Illalla teen ruokaa, luen kirjaa ja menen nukkumaan kello yksitoista.","Mitä henkilö tekee illalla?",["Tekee ruokaa","Menee töihin","Matkustaa","Ui"],"Tekee ruokaa")]
    grammar_bank=[(["Minulla ei ole auto.","Minulla ei ole autoa.","Minulla ei ole auton."],"Minulla ei ole autoa."),(["Minä juon kahvi.","Minä juon kahvia.","Minä juon kahvin."],"Minä juon kahvia."),(["Syön kaksi omenaa.","Syön kaksi omenat.","Syön kaksi omenan."],"Syön kaksi omenaa."),(["Menen kauppaan.","Menen kaupassa.","Menen kaupasta."],"Menen kauppaan."),(["Olen Helsingissä.","Olen Helsinkiin.","Olen Helsingistä."],"Olen Helsingissä."),(["Hän ei puhu suomea.","Hän ei puhu suomi.","Hän ei puhu suomen."],"Hän ei puhu suomea.")]
    vocab_sets=[(["maito","juna","leipä","kahvi","opettaja","omena"],{"maito","leipä","kahvi","omena"},"Select food/drink words:"),(["bussi","juna","omena","metro","kirja","pyörä"],{"bussi","juna","metro","pyörä"},"Select transport words:"),(["aamu","ilta","kauppa","yö","päivä","auto"],{"aamu","ilta","yö","päivä"},"Select time-of-day words:"),(["opettaja","lääkäri","banaani","tutkija","pappi","metro"],{"opettaja","lääkäri","tutkija","pappi"},"Select professions:"),(["keittiö","makuuhuone","juna","olohuone","kylpyhuone","kahvi"],{"keittiö","makuuhuone","olohuone","kylpyhuone"},"Select rooms/home words:")]
    writing_prompts=["Write 3–4 simple Finnish sentences about yourself.","Write 3–5 sentences about your family.","Write 4 sentences about your Finnish class day.","Write 4–5 sentences about what you do in the morning.","Write a short Finnish message to a friend: suggest meeting tomorrow.","Describe your home in 4–5 simple sentences."]
    speaking_prompts=["Kerro itsestäsi: kuka olet, missä asut ja mitä opiskelet?","Kerro perheestäsi.","Kerro tavallisesta aamustasi.","Kerro mitä teet tänään.","Kerro suomen kurssistasi.","Kerro viikonlopustasi."]
    tabs=st.tabs(["📖 Reading","✍️ Writing","🎧 Listening","🎙️ Speaking","🧩 Grammar","🧠 Vocabulary"]);seed=date.today().toordinal()
    with tabs[0]:
        r=st.session_state.attempts["Reading"]+1;item=reading_bank[(seed+r-1)%len(reading_bank)];st.caption(f"Round {r} · Continue as long as you want today.");st.write("**Teksti:** "+item[0]);ans=st.radio(item[1],item[2],index=None,key=f"read_{date.today()}_{r}")
        if st.button("Submit reading round",key=f"read_btn_{r}"):save_score("Reading",100 if ans==item[3] else 0);st.rerun()
    with tabs[1]:
        r=st.session_state.attempts["Writing"]+1;st.caption(f"Round {r}");txt=st.text_area(writing_prompts[(seed+r-1)%len(writing_prompts)],key=f"write_{r}")
    ...
    with tabs[2]:
        r=st.session_state.attempts["Listening"]+1;item=listen_bank[(seed+r-1)%len(listen_bank)];st.caption(f"Round {r}");sentence=item[0].replace("'","\\'");components.html(f'''<button onclick="speechSynthesis.cancel();let u=new SpeechSynthesisUtterance('{sentence}');u.lang='fi-FI';u.rate=.78;speechSynthesis.speak(u);">▶ Play Finnish</button>''',height=45);ans=st.radio(item[1],item[2],index=None,key=f"listen_{r}")
        if st.button("Submit listening round",key=f"listen_btn_{r}"):save_score("Listening",100 if ans==item[3] else 0);st.rerun()
    with tabs[3]:
        r=st.session_state.attempts["Speaking"]+1;st.caption(f"Round {r}");st.write("Speak for about 20–45 seconds: **"+speaking_prompts[(seed+r-1)%len(speaking_prompts)]+"**");spoken=speech_to_text(language="fi",start_prompt="🎙️ Start speaking Finnish",stop_prompt="⏹️ Stop & transcribe",just_once=False,key=f"rukhsar_asr_{r}")
        if spoken:
    ...
    with tabs[4]:
        r=st.session_state.attempts["Grammar"]+1;item=grammar_bank[(seed+r-1)%len(grammar_bank)];st.caption(f"Round {r}");ans=st.radio("Choose the correct sentence:",item[0],index=None,key=f"gram_{r}")
    ...
    with tabs[5]:
        r=st.session_state.attempts["Vocabulary"]+1;item=vocab_sets[(seed+r-1)%len(vocab_sets)];st.caption(f"Round {r}");chosen=set(st.multiselect(item[2],item[0],key=f"vocab_{r}"))
    ...
    st.header("📈 Finnish Progress · Goal A2.2");h=st.session_state.finnish_history;rows=[{"Date":pd.to_datetime(d),"Overall":x.get("overall",0),**x.get("scores",{})} for d,x in sorted(h.items())];df=pd.DataFrame(rows);overall=round(sum(st.session_state.scores.values())/6);c=st.columns(4);c[0].metric("Current estimate",level(overall));c[1].metric("Target","A2.2");c[2].metric("Practice days",len(h));c[3].metric("Exercises",sum(x.get("tasks_completed",0) for x in h.values()));st.line_chart(df.set_index("Date"),height=300) if not df.empty else st.info("Complete Finnish exercises to start the progress history.")
    ...
    st.dataframe(rows,use_container_width=True,hide_index=True,column_config={"Official Link":st.column_config.LinkColumn("Official Link",display_text="Open")}) if rows else st.info("No applications tracked yet.")
